...
try:
    ...
    for i, result in enumerate(entry_archive.data.results):
        ...
        if i < 2:  # Show first few data points for first 2 spectra
            print(f"     - Raman shift range: {min(result.raman_shift)} - {max(result.raman_shift)}")
            print(f"     - Intensity range: {min(result.intensity)} - {max(result.intensity)}")
    ...
    print("OR")
    print("  pytest tests/schema_packages/test_measurements.py::test_schema[raman] -v -s")
