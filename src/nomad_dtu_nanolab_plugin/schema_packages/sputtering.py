...
class DTUSamples(CompositeSystemReference, ArchiveSection):
    ...
    def normalize(self, archive: 'EntryArchive', logger: 'BoundLogger') -> None:
        ...
        if self.sub_xpos is None and self.sub_ypos is None:
            positions = {
                'BL': (-0.02, 0.035),
                'BR': (0.02, 0.035),
                'FL': (-0.02, -0.005),
            ...
            if self.relative_position in positions:
                self.sub_xpos, self.sub_ypos = positions[self.relative_position]
...
class Chamber(ArchiveSection):
    ...
    average_platen_voltage = Quantity(
        type=np.float64,
        a_eln={'component': 'NumberEditQuantity', 'defaultDisplayUnit': 'V'},
        unit='V',
    )
...
class Special(ArchiveSection):
    ...
    m_def = Section()
    platen_temp_ramp_rate = Quantity(
        type=np.float64,
        default=0.3333,
...
class DTUSputterParameters(ArchiveSection):
    """
    Class autogenerated from yaml schema.
    """
    ...
    def normalize(self, archive: 'EntryArchive', logger: 'BoundLogger') -> None:
        """
        The normalizer for the `DTUSputterParameters` class.
...
class DTUSteps(PVDStep, ArchiveSection):
    ...
    sputter_parameters = SubSection(
        section_def=DTUSputterParameters,
        repeats=True,
    )
