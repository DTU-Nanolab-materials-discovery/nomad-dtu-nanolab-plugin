...
class DTUSputtering(SputterDeposition, Schema):
    ...
    def normalize(self, archive: 'EntryArchive', logger: 'BoundLogger') -> None:
        ...
        # Check if self.lab_id is not None
        if self.lab_id is not None:
            for samples in self.DTUsamples:
                if self.DTUsamples.relative_position is not None:
                    if self.DTUsamples.lab_id is None:
                    # Concatenate self.lab_id with the subsection's relative_position
                    # Ensure both are converted to strings in case they are not
                        name = str(self.lab_id) + str(self.DTUsample.relative_position)
                    # Assign the concatenated string as the lab_id of the subsection
                        self.DTUsample.lab_id = name
        #ToDos:
