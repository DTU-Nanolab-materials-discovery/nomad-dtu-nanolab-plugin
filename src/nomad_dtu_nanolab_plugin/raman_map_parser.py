...
class MappingRamanMeas:
            ...
            if (
                ...
                for i in range(len(reader.xpos)):
                    ...
                    if len(spectra_array.shape) == 2:  # noqa: PLR2004
                        ...
                        if i < NBR_SPECTRA:  # Debug first few spectra
                            print(
                                f'Spectrum {i}: '
    ...
    def plot_spectra(
        self, color_by='x_pos', method='normalize', x_range=None, max_spectra=None
    ):
        if method == 'normalize':
...
def main():
    ...
    for i, raman_meas in enumerate(mapping.raman_meas_list[:3]):
        print(
            f'Point {i}: x={raman_meas.x_pos:.1f}, y={raman_meas.y_pos:.1f}, '
            f'has_image={raman_meas.image is not None}, '
