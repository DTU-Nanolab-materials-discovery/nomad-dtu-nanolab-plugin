...
@click.option(
    ...
    default=None,
    help='Full path to WDF file. Alternative to --folder/--filename.',
)
@click.option(
    '--folder',
    '-f',
    ...
    default=None,
    help='Directory containing the WDF file(s). Use with --filename.',
)
@click.option(
    '--filename',
    '-n',
    default=None,
    help='Name of the WDF file to process. Use with --folder.',
)
@click.option(
    '--output-dir',
    '-o',
    ...
    default=None,
    help='Directory for output files. Defaults to same as input folder.',
)
@click.option(
    '--target-wavenumber',
    '-w',
    type=float,
    default=None,
    help='Target Raman shift in cm⁻¹ for intensity map. Auto-detects if not specified.',
)
@click.option(
    ...
    type=float,
    default=5.0,
    help='Tolerance window around target wavenumber in cm⁻¹. Default: 5.0',
)
@click.option(
    '--x-range',
    '-x',
    type=(float, float),
    default=None,
    help='Wavenumber range for spectra plot as "MIN MAX" (e.g., 80 1000).',
)
@click.option(
    '--save-images/--no-save-images',
    default=True,
    help='Save individual optical images to disk. Default: True',
)
@click.option(
    '--create-grid/--no-create-grid',
    default=True,
    help='Create optical image grid. Default: True',
)
@click.option(
    '--verbose',
    '-v',
...
def main(  # noqa: PLR0913
    ...
    """Process Raman mapping WDF files and generate visualizations.

    This CLI tool reads Renishaw WDF files containing Raman mapping data,
    extracts spectral data and optical images, and generates interactive
    visualizations including spectral plots and intensity maps.

    You can specify the input file in two ways:

    \b
    1. Using --file with full path:
       --file /path/to/sample.wdf

    \b
    2. Using --folder and --filename separately:
       --folder /path/to --filename sample.wdf

    Examples:

        \b
        # Using full file path
        python raman_map_parser.py --file ./data/sample_map.wdf

        \b
        # Using folder and filename separately
        python raman_map_parser.py -f ./data -n sample_map.wdf

        \b
        # With verbose output and custom wavenumber
        python raman_map_parser.py --file ./data/sample.wdf -w 380 -v

        \b
        # Custom output directory and wavenumber range
        python raman_map_parser.py --file ./data/sample.wdf -o ./results -x 100 1000
    ...
    if verbose:
        ...
        click.echo(click.style('Raman Mapping Analysis', fg='cyan', bold=True))
        click.echo(click.style('=' * 60, fg='cyan'))
        click.echo(f'Input folder:  {folder}')
        click.echo(f'Output folder: {output_folder}')
        click.echo(f'WDF file:      {filename}')
    ...
    if verbose:
        click.echo(f'\nLoaded {len(mapping.raman_meas_list)} Raman spectra')

    # Save individual optical images
    if save_images:
        if verbose:
            click.echo(click.style('\nSAVING OPTICAL IMAGES', fg='green', bold=True))
            click.echo('=' * 60)
        mapping.save_optical_images(output_folder, meas_name, verbose=verbose)

    # Create image grid
    if create_grid:
        if verbose:
            click.echo(click.style('\nCREATING IMAGE GRID', fg='green', bold=True))
            click.echo('=' * 60)
        grid_path = os.path.join(output_folder, f'{meas_name}_optical_grid.png')
        fig = mapping.create_image_grid(save_path=grid_path, verbose=verbose)
        if fig and verbose:
            click.echo(f'Saved grid to: {grid_path}')
    ...
    map_fig = mapping.plot_intensity_map(
        ...
        wavenumber_tolerance=wavenumber_tolerance,
        verbose=verbose,
    )
    ...
    if verbose:
        ...
        click.echo(f'  • {meas_name}_Raman_spectra.html')
        click.echo(f'  • {meas_name}_Raman_intensity_map.html')
        if create_grid:
            click.echo(f'  • {meas_name}_optical_grid.png')
        if save_images:
            img_count = sum(1 for m in mapping.raman_meas_list if m.image is not None)
            click.echo(f'  • {img_count} individual optical images')
