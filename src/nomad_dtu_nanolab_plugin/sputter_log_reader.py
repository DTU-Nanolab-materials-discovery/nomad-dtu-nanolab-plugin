...
for source_number in ['1', '3', '4']:
    ...
    COL[f"s{source_number}_pulse_freq_sp"] = f"Source {source_number} Pulse Frequency Setpoint"
    COL[f"s{source_number}_pulse_freq"] = f"Source {source_number} Pulse Frequency"
    COL[f"s{source_number}_rev_time_sp"] = f"Source {source_number} Reverse Time Setpoint"
...
def generate_bias_plot(
        ...
        if any(re.search(pattern, col) for pattern in patterns):
            ...
            if '_Sb_' in logfile_name:
                ...
                deposition.data[f'{col} No Zero'] = (
                    deposition.data[col].replace(0, np.nan))
    ...
    bias_plot = quick_plot(
        deposition.data, Y_plot, mode='default', plot_type='line',
        width= WIDTH,
